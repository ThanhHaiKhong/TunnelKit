...
extension OpenVPN {
    ...
    public class ConfigurationParser {
        ...
        public struct Regex {
            ...
            static let setenv = NSRegularExpression("^setenv +(\\S+) +(.+)")
...
//            static let fragment = NSRegularExpression("^fragment +\\d+")
        ...
        public static func parsed(
            ...
            var optSetenvVars: [String: String] = [:]
            ...
            for line in lines {
                ...
                Regex.cipher.enumerateSpacedArguments(in: line) {
                    ...
                    // Handle "none" specially (it's lowercase in the enum)
                    if rawValue.lowercased() == "none" {
                        optCipher = Optional<Cipher>(.none)
                    } else {
                ...
                Regex.xorInfo.enumerateSpacedArguments(in: line) {
                    ...
                    case "xormask":
                        if $0.count > 1 {
                            // Use mask as UTF8 string (not hex decoded) to match working behavior
                            let mask = Data($0[1].utf8)
                ...
                Regex.setenv.enumerateSpacedArguments(in: line) {
                    isHandled = true
                    guard $0.count >= 2 else {
                        return
                    }
            ...
            if !optSetenvVars.isEmpty {
                sessionBuilder.templateSetenvVars = optSetenvVars
            }
